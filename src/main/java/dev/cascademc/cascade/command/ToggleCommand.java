...
public class ToggleCommand {
    ...
    public ToggleCommand(ScriptManager scriptManager) {
    ...
    private int execute(CommandContext<FabricClientCommandSource> context) {
        ...
        try {
            boolean wasEnabled = scriptManager.isEnabled(scriptId);
            scriptManager.toggleScript(scriptId);

            String status = wasEnabled ? "Disabled" : "Enabled";
            sendMessage(context, Component.text(status + " script: ", NamedTextColor.GREEN)
                    .append(Component.text(scriptId, NamedTextColor.YELLOW)));
            return 1;
        } catch (Exception e) {
            sendMessage(context, Component.text("Failed to toggle script: " + scriptId, NamedTextColor.RED));
            CascadeClient.LOGGER.error("Failed to toggle script via command", e);
            return 0;
        }
    }
